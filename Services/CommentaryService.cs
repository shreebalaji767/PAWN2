...
public sealed class CommentaryService
{
    ...
    public string ForPlayerMove(Move m, ComputerProfile p) =>
        m.Captured != null
            ? Capture[rng.Next(Capture.Length)]
            : Bad[rng.Next(Bad.Length)];

    public string ForComputerMove(Move m, ComputerProfile p) =>
        m.Captured != null
            ? Capture[rng.Next(Capture.Length)]
            : p.Ragebait > 75
                ? RageLines[rng.Next(RageLines.Length)]
                : "Did you see that? I absolutely planned that. Do not inspect the code.";
    ...
    public IEnumerable<string> GameOver(bool playerWon, string result)
    {
        ...
        else
        {
            ...
            yield return "I WON. Someone please write this down. I need evidence.";
        }
    }
}
